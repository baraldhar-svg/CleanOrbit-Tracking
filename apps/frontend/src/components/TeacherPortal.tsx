    ...
    <div className="mx-auto max-w-4xl p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-xl bg-amber-650 hover:bg-amber-600 disabled:bg-muted disabled:text-muted-foreground px-4 py-2.5 text-sm font-bold text-white transition-colors cursor-pointer"
        >
          <Bus size={16} />
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 space-y-3 bg-muted/10 border border-border rounded-2xl">
            <RefreshCw className="animate-spin text-amber-500" size={32} />
            <p className="text-muted-foreground text-sm">Retrieving student records...</p>
          </div>
        ) : filteredStudents.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 space-y-2 bg-muted/10 border border-border rounded-2xl text-center px-4">
            <Users className="text-muted-foreground" size={40} />
            <h3 className="text-foreground font-bold">No students found</h3>
          ...
          <div className="bg-card border border-border rounded-2xl divide-y divide-border overflow-hidden shadow-sm">
            {filteredStudents.map((student) => {
              ...
              return (
                <div key={student.studentId} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-muted/10 transition-colors">
                  ...
                  <div className="flex items-center gap-3">
                    <img
                      ...
                      className="h-11 w-11 rounded-xl bg-background object-cover border border-border shrink-0"
                    />
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground mt-0.5">
                        <span>Bus Stop:</span>
                        <span className="text-amber-600 dark:text-amber-500 font-semibold">{student.stationName || "No Bus"}</span>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 px-3.5 py-2 rounded-xl text-xs font-bold transition-all border cursor-pointer ${
                        student.classStatus === 'PRESENT'
                          ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 border-emerald-500/40 shadow-[0_0_12px_rgba(16,185,129,0.15)]'
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 px-3.5 py-2 rounded-xl text-xs font-bold transition-all border cursor-pointer ${
                        student.classStatus === 'ABSENT'
                          ? 'bg-rose-500/20 text-rose-600 dark:text-rose-300 border-rose-500/40 shadow-[0_0_12px_rgba(244,63,94,0.15)]'
                          : 'bg-background text-muted-foreground border-border hover:border-accent hover:text-foreground'
                      }`}
                    >
                      <XCircle size={14} />
