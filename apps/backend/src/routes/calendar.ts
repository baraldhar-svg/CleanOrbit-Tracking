...
router.post("/", async (req, res) => {
  ...
  // Prevent creating duplicate events for the same title and date
  const [existing] = await db
    ...
    .where(
      and(
        eq(calendarEventsTable.tenantId, req.tenantId),
        eq(calendarEventsTable.eventDate, eventDate),
        eq(calendarEventsTable.title, title)
      )
    )
    .limit(1);
...
// POST /calendar-events/preload-national-holidays
router.post("/preload-national-holidays", async (req, res) => {
  try {
      ...
      const [existing] = await db
        ...
        .where(
          and(
            ...
            eq(calendarEventsTable.eventDate, h.eventDate)
          )
        )
        .limit(1);
    ...
    return res.json({ success: true, addedCount, message: `Preloaded ${addedCount} Nepal national holidays` });
  } catch (err: any) {
    return res.status(500).json({ error: err?.message || "Failed to preload holidays" });
  }
});
