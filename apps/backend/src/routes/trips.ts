...
// GET /api/trips/locations — returns ALL drivers with their real GPS positions.
...
router.get("/timeline", async (req, res) => {
  try {
    ...
    const conditions = [eq(tripLogsTable.tenantId, tenantId)];
    if (routeIdParam) {
      conditions.push(eq(tripLogsTable.routeId, routeIdParam));
      ...
      if (isTodayTrip) {
        ...
        for (const ann of announcements) {
          ...
          if (annTime >= todayStart) {
            const isBusStartAnn = ann.message.includes("journey started");
            if (isBusStartAnn) {
              if (latestTrip.vehicleNumber && ann.message.includes(latestTrip.vehicleNumber)) {
                events.push({
                  id: eventId++,
    ...
    if (events.length === 0) {
      for (const ann of announcements) {
        const annTime = new Date(ann.createdAt);
        if (annTime >= todayStart && !ann.message.includes("journey started")) {
          events.push({
            id: eventId++,
            time: annTime.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true }),
            description: `📢 Notice: ${ann.message}`,
