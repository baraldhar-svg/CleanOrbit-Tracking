...
router.post("/pending-registrations/:id/approve", async (req, res) => {
  ...
  // Send approval email — MUST await on serverless so Vercel does not terminate process before SMTP completes!
  const baseUrl = process.env.APP_URL || "https://clean-orbit-tracking.vercel.app";
  const verifyLink = `${baseUrl}/admin-verify?code=${schoolCode}&mobile=${encodeURIComponent(reg.mobile)}`;
  let emailSent = false;
  try {
    const mailRes = await sendSchoolApprovalEmail(reg.email.trim().toLowerCase(), reg.schoolName, reg.adminName, schoolCode, verifyLink);
    emailSent = mailRes.success;
  } catch (err) {
    console.error("Failed to send approval email:", err);
  }

  return res.json({ registration: updated, tenant, schoolCode, emailSent });
});

// POST /api/superadmin/approve-school (Standalone school approval endpoint)
router.post("/approve-school", async (req, res) => {
  try {
    const { schoolId, schoolEmail, schoolName, adminName, schoolCode, mobileNumber } = req.body;

    const id = Number(schoolId);
    if (!isNaN(id)) {
      await db
        .update(adminRegistrationsTable)
        .set({ status: "APPROVED", schoolCode })
        .where(eq(adminRegistrationsTable.id, id));
    }

    const baseUrl = process.env.APP_URL || "https://clean-orbit-tracking.vercel.app";
    const verifyLink = `${baseUrl}/admin-verify?code=${schoolCode}&mobile=${encodeURIComponent(mobileNumber || "")}`;

    const emailResult = await sendSchoolApprovalEmail(
      schoolEmail,
      schoolName,
      adminName,
      schoolCode,
    ...
    if (emailResult.success) {
      return res.status(200).json({
        success: true,
        message: "School approved and confirmation email sent successfully!",
      });
    } else {
      return res.status(500).json({
        success: false,
        message: "School approved in database, but failed to send email.",
      });
    }
  } catch (err: any) {
...
// POST /api/superadmin/pending-registrations/:id/resend-email
router.post("/pending-registrations/:id/resend-email", async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
    ...
    const [reg] = await db.select().from(adminRegistrationsTable).where(eq(adminRegistrationsTable.id, id)).limit(1);
    if (!reg) return res.status(404).json({ error: "Registration not found" });
    if (!reg.schoolCode) return res.status(400).json({ error: "School code not generated yet" });

    const baseUrl = process.env.APP_URL || "https://clean-orbit-tracking.vercel.app";
    const verifyLink = `${baseUrl}/admin-verify?code=${reg.schoolCode}&mobile=${encodeURIComponent(reg.mobile)}`;
    ...
    return res.json({ success: result.success, message: result.success ? "Approval email sent successfully!" : "Failed to send approval email." });
  } catch (err: any) {
    return res.status(500).json({ success: false, error: err?.message || "Failed to resend email" });
  }
});
